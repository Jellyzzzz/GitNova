...
public record AgentRunResult(
    ...
    public AgentRunResult {
        ...
        if (status == AgentRunStatus.COMPLETED) {
            ...
            if (reviewDraft != null) {
                throw new IllegalArgumentException("non-completed run must not contain reviewDraft");
            }
            if (terminationReason == AgentTerminationReason.FINALIZE_SUCCEEDED) {
                throw new IllegalArgumentException("only completed run can finalize successfully");
            }
        }

        if (status == AgentRunStatus.PARTIAL && !coverage.hasEvidence()) {
            throw new IllegalArgumentException("partial run must contain review coverage");
        }
        if (status == AgentRunStatus.FAILED && coverage.hasEvidence()) {
            throw new IllegalArgumentException("failed run must not contain review coverage");
        }
